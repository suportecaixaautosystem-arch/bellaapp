...
import { CheckCircle, Calendar as CalendarIcon, Plus, X, Scissors, Briefcase, Tag, Search, Loader } from 'lucide-react';
...
const NewAppointment: React.FC = () => {
    ...
    const editingAppointmentId = location.state?.appointmentId as number | undefined;

    const [loading, setLoading] = useState(true);
    const [services, setServices] = useState<Service[]>([]);
    const [serviceCombos, setServiceCombos] = useState<(ServiceCombo & { service_combo_items: { service_id: number }[] })[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const combosPromise = supabase.from('service_combos').select('*, service_combo_items(service_id)').eq('is_active', true);
            const employeesPromise = supabase.from('employees').select('*, employee_services(service_id)').eq('is_active', true);
            ...
            const companyPromise = supabase.from('companies').select('*').eq('id', 1).limit(1);

            const [
                { data: servicesData, error: servicesError },
                { data: combosData, error: combosError },
                { data: employeesData, error: employeesError },
                { data: clientsData, error: clientsError },
                { data: appointmentsData, error: appointmentsError },
                { data: companyResult, error: companyError },
            ] = await Promise.all([servicesPromise, combosPromise, employeesPromise, clientsPromise, appointmentsPromise, companyPromise]);

            const errors = { servicesError, combosError, employeesError, clientsError, appointmentsError, companyError };
            const hasError = Object.values(errors).some(e => e && e.code !== 'PGRST116');

            if (hasError) {
                console.error(errors);
                alert('Erro ao carregar dados necessários para o agendamento.');
            } else {
                setServices(servicesData || []);
                setServiceCombos(combosData as any || []);
                setEmployees(employeesData as any || []);
                setClients(clientsData || []);
                setAllAppointments(appointmentsData || []);
                setCompany(companyResult && companyResult.length > 0 ? companyResult[0] : null);
            }
            setLoading(false);
        };
        fetchData();
    ...
    const selectableItems: SelectableItem[] = useMemo(() => {
        const serviceItems = services.map(s => ({
            id: `service-${s.id}`, name: s.name, type: 'service' as const, price: s.price, duration: s.duration_minutes, serviceIds: [s.id]
        }));
        const comboItems = serviceCombos.map(c => {
            const comboServices = services.filter(s => c.service_combo_items.some((sci: any) => sci.service_id === s.id));
            const totalDuration = comboServices.reduce((acc, s) => acc + s.duration_minutes, 0);
            return {
                id: `combo-${c.id}`, name: c.name, type: 'combo' as const, price: c.price, duration: totalDuration, serviceIds: c.service_combo_items.map((sci: any) => sci.service_id)
            }
        });
        return [...comboItems, ...serviceItems];
    }, [services, serviceCombos]);

    const filteredSelectableItems = useMemo(() => {
        if (!itemSearchTerm) return selectableItems;
        return selectableItems.filter(item => item.name.toLowerCase().includes(itemSearchTerm.toLowerCase()));
    }, [selectableItems, itemSearchTerm]);

    const handleItemSelection = (itemId: string) => {
        setFormState(prev => ({
            ...prev,
            selectedItemIds: prev.selectedItemIds.includes(itemId) ? prev.selectedItemIds.filter(id => id !== itemId) : [...prev.selectedItemIds, itemId],
            employeeId: '', time: '',
        }));
    };

    const selectedItems = useMemo(() => selectableItems.filter(item => formState.selectedItemIds.includes(item.id)), [formState.selectedItemIds, selectableItems]);
    const { totalDuration, totalPrice } = useMemo(() => selectedItems.reduce((acc, item) => ({
        totalDuration: acc.totalDuration + item.duration,
        totalPrice: acc.totalPrice + item.price
    }), { totalDuration: 0, totalPrice: 0 }), [selectedItems]);
    ...
    const availableEmployees = useMemo(() => {
        ...
        const companyWorkingHours = (company?.working_hours as any[]) || defaultCompanyWorkingHours;
        ...
        return employees.filter(emp => {
            const empSchedule = (emp.working_hours as any[])?.find(d => d.day === dayName);
            const canPerformAllServices = requiredServiceIds.every(serviceId => emp.employee_services.some(es => es.service_id === serviceId));
            return emp.is_active && empSchedule?.active && canPerformAllServices;
        });
    }, [formState.date, selectedItems, employees, company]);

    const availableSlots = useMemo(() => {
        if (!formState.date || !formState.employeeId || selectedItems.length === 0) return [];
        const employee = employees.find(e => e.id === parseInt(formState.employeeId));
        if (!employee) return [];

        const companyWorkingHours = (company?.working_hours as any[]) || defaultCompanyWorkingHours;
        const dayName = dayIndexToName[formState.date.getDay()];
        const companySchedule = companyWorkingHours.find(d => d.day === dayName);
        const employeeSchedule = (employee.working_hours as any[])?.find(d => d.day === dayName);
        
        if (!companySchedule?.active || !employeeSchedule?.active) return [];
        
        const slots: string[] = [];
        const appointmentsOnDate = allAppointments.filter(apt => apt.employee_id === employee.id && new Date(apt.start_time).toDateString() === formState.date.toDateString() && apt.status === 'agendado' && apt.id !== editingAppointmentId);
        ...
        for (let t = workStart; t <= workEnd - totalDuration; t += 15) {
            ...
            for (const apt of appointmentsOnDate) {
                const aptStart = new Date(apt.start_time).getHours() * 60 + new Date(apt.start_time).getMinutes();
                const aptEnd = new Date(apt.end_time).getHours() * 60 + new Date(apt.end_time).getMinutes();
        ...
        return slots;
    }, [formState.date, formState.employeeId, selectedItems, allAppointments, editingAppointmentId, totalDuration, employees, company]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!clientId || selectedItemIds.length === 0 || !employeeId || !time) {
        ...
        if (appointmentError || !appointmentData) {
            alert(`Erro ao criar agendamento: ${appointmentError?.message}`); return;
        }

        const itemsToInsert = selectedItems.map(item => ({
            appointment_id: appointmentData.id,
            service_id: item.type === 'service' ? parseInt(item.id.split('-')[1]) : null,
            combo_id: item.type === 'combo' ? parseInt(item.id.split('-')[1]) : null,
            price: item.price,
            duration_minutes: item.duration,
        }));

        const { error: itemsError } = await supabase.from('appointment_items').insert(itemsToInsert as any);

        if (itemsError) {
            alert(`Erro ao adicionar itens ao agendamento: ${itemsError.message}`);
    ...
    // Client Modal Logic would go here

    if (loading) {
        return <div className="text-center p-8 flex items-center justify-center space-x-2"><Loader className="animate-spin h-5 w-5" /><span>Carregando dados...</span></div>;
    }
    ...
    return (
        <div className="space-y-6">
            <div><h1 className="text-2xl lg:text-3xl font-bold text-white">{editingAppointmentId ? 'Editar Agendamento' : 'Novo Agendamento'}</h1><p className="text-gray-400 mt-1">Preencha os detalhes para criar um novo horário</p></div>
